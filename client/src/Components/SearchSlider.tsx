...
function Slider({ movies, title, category, keyword, section }: ISliderProps) {
  ...
  const bigMovieMatch = useMatch(
    category === "영화"
      ? `/search/movies/${section}/:movieId`
      : `/search/tv/${section}/:movieId`
  );
          ...
          <>
            ...
            <BigMovie layoutId={`${bigMovieMatch.params.movieId}${section}`}>
              {clickedMovie && (
                <>
                  <BigCover
                    bg={makeImgPath(clickedMovie.backdrop_path, "w500")}
                  >
                    <BigPoster
                      bg={makeImgPath(clickedMovie.poster_path, "w500")}
                    />
                    <BigCoverInfo>
                      <BigCoverGenres>
                        {category === "영화"
                    ...
                    {category === "영화"
                      ? castMovieData?.cast.slice(0, 5).map((actor, index) => {
                          return (
                            <div key={index}>
                              <BigCastImg
                                bg={makeImgPath(actor.profile_path, "w200")}
                              />
